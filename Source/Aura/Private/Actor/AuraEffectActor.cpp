...
void AAuraEffectActor::ApplyEffectToTarget(AActor* TargetActor, TSubclassOf<UGameplayEffect> GameplayEffectClass)
{
	...
	if (bIsInfinite && InfiniteEffectRemovalPolicy == EEffectRemovalPolicy::RemoveOnEndOverlap)
	{
		...
		InfiniteActiveEffectHandles.Add(UID, ActiveEffect);
	}
}
...
void AAuraEffectActor::OnEndOverlap(AActor* TargetActor)
{
	...
	if (InfiniteEffectRemovalPolicy == EEffectRemovalPolicy::RemoveOnEndOverlap)
	{
		...
		const auto UID = TargetActor->GetUniqueID();
		if (!InfiniteActiveEffectHandles.Contains(UID)) return;

		const auto& EffectHandle = InfiniteActiveEffectHandles[UID];
		if (!TargetASC->RemoveActiveGameplayEffect(EffectHandle, 1)) return;

		InfiniteActiveEffectHandles.Remove(UID);
	}
}
